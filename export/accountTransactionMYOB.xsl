...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	...
	<ish:onCourse>
		<ish:name>MYOB</ish:name>
		<ish:entity>AccountTransaction</ish:entity>
		<ish:version>1</ish:version>
		<ish:keyCode>ish.onCourse.accountTransactionMYOB</ish:keyCode>
	</ish:onCourse>
	...
	<xsl:param name="zero" select="'0.00'" />
	
	<xsl:template match="*">
		<xsl:value-of select="replace(normalize-space(), $quote, $doublequote)" />
	...
	<xsl:function name="ish:outputValue">
		...
		<xsl:value-of select="replace(normalize-space($value), $quote, $doublequote)" />
	...
	<xsl:template match="data">
		<xsl:text>Date&#x9;Memo&#x9;Account Number&#x9;Debit Amount&#x9;Credit Amount</xsl:text>
				<xsl:value-of select="$break" />
		<xsl:apply-templates select="accountTransaction" />
	</xsl:template>
	
	<xsl:template match="accountTransaction">
		<xsl:value-of select="ish:outputValue(format-dateTime(transactionDate, '[D]/[M]/[Y]'))"/>
		<xsl:value-of select="ish:outputValue(transactionDescription)"/>
		<xsl:apply-templates select="account"/>
		<xsl:choose>
    		<xsl:when test="amount > 0.0">
    			<xsl:value-of select="ish:outputValue(amount)"/>
    			<xsl:value-of select="ish:outputValue($zero, true())"/>
    		</xsl:when>
    		<xsl:otherwise>
    			<xsl:value-of select="ish:outputValue($zero)"/>
    			<xsl:value-of select="ish:outputValue(amount, true())"/>
    		</xsl:otherwise>
		</xsl:choose>

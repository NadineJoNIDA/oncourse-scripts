...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	...
	<ish:onCourse>
		...
		<ish:entity>PaymentIn</ish:entity>
		<ish:version>12</ish:version>
		<ish:keyCode>ish.onCourse.paymentInCsv</ish:keyCode>
	</ish:onCourse>
	...
	<xsl:template match="data">
		<xsl:text>id,payerLastName,payerFirstName,payerEmail,modifiedOn,createdOn,amount,dateBanked,reconciled,source,status,type,creditCardCVV,creditCardExpiry,creditCardName,creditCardNumber,creditCardType,gatewayReference,gatewayResponse,chequeBank,chequeBranch,chequeDrawer,privateNotes</xsl:text>
		<xsl:value-of select="$break" />
		<xsl:apply-templates select="paymentIn" />
	</xsl:template>
	
	<xsl:template match="paymentIn">
		...
		<xsl:value-of select="ish:outputValue(gatewayResponse)" />

	...
	map.each { Contact c, List<Payslip> payslips ->
		contact(id: c.id) {
			...
			payslips.each { Payslip ps ->
				tutorPay(id: ps.id) {
					modifiedOn(ps.modifiedOn?.format("yyyy-MM-dd'T'HH:mm:ssZ"))
					createdOn(ps.createdOn?.format("yyyy-MM-dd'T'HH:mm:ssZ"))
					status(ps.status)

					ps.paylines.each { PayLine pl ->
						payline() {
							modifiedOn(pl.modifiedOn?.format("yyyy-MM-dd'T'HH:mm:ssZ"))
							createdOn(pl.createdOn?.format("yyyy-MM-dd'T'HH:mm:ssZ"))
							...
							dateFor(pl.dateFor?.format("yyyy-MM-dd'T'HH:mm:ssZ"))
							...
							if (pl.session) {
								session() {
									modifiedOn(pl.session.modifiedOn?.format("yyyy-MM-dd'T'HH:mm:ssZ"))
									createdOn(pl.session.createdOn?.format("yyyy-MM-dd'T'HH:mm:ssZ"))
									start(pl.session.startDatetime?.format("yyyy-MM-dd'T'HH:mm:ssZ"))
									end(pl.session.endDatetime?.format("yyyy-MM-dd'T'HH:mm:ssZ"))

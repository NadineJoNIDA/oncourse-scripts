...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	...
	<ish:onCourse>
		...
		<ish:entity>Room</ish:entity>
		<ish:version>8</ish:version>
		<ish:keyCode>ish.onCourse.roomCsv</ish:keyCode>
	</ish:onCourse>
	...
	<xsl:template match="data">
		<xsl:text>name,seatedCapacity,directions,facilities,notes,createdOn,modifiedOn,siteName,siteIsAdministrationCentre,siteLatitude,siteLongitude,sitePostcode,siteState,siteStreet,siteSuburb,siteCreatedOn,siteModifiedOn</xsl:text>
		<xsl:value-of select="$break" />
		<xsl:apply-templates select="site/room" />
	</xsl:template>
	
	<xsl:template match="room">
		<xsl:value-of select="ish:outputValue(name)" />
		<xsl:value-of select="ish:outputValue(seatedCapacity)" />
		<xsl:value-of select="ish:outputValue(directions)" />
		<xsl:value-of select="ish:outputValue(facilities)" />

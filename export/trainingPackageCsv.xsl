...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	...
	<ish:onCourse>
		...
		<ish:entity>TrainingPackage</ish:entity>
		<ish:version>5</ish:version>
		<ish:keyCode>ish.onCourse.trainingPackageCsv</ish:keyCode>
	</ish:onCourse>
	...
	<xsl:template match="data">
		<xsl:text>nationalISC,title,modifiedOn,createdOn</xsl:text>
				<xsl:value-of select="$break" />
		<xsl:apply-templates select="trainingPackage" />
	</xsl:template>
	
	<xsl:template match="trainingPackage">
		<xsl:value-of select="ish:outputValue(nationalISC)" />
		<xsl:value-of select="ish:outputValue(title)" />
		<xsl:value-of select="ish:outputValue(modifiedOn)" />
		<xsl:value-of select="ish:outputValue(createdOn, true())" />
		<xsl:if test="following-sibling::*">
			<xsl:value-of select="$break" />
		</xsl:if>
	</xsl:template>
